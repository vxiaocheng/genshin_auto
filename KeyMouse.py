...
# 设置鼠标的位置并点击
def setMouseAndClick(x: int, y: int, type: str = 'left', seconds: float = 0.1):
    setMousePosition(x, y)
    keepMousePress(type, seconds)
...
def keepKeyPress(key, seconds: float = 0.1, delay=0.02):
    t = time.time()
    print(t)
    print(my_keyboard)
...
# 播放进程
def recordThread(file):
    t1 = keyboardListenerBreak()
    t2 = threading.Thread(target=executeRecord, args=(file,))
...
if __name__ == '__main__':
    ...
    # recordThread(f)
    t1 = keyboardListenerBreak()
    t2 = threading.Thread(target=executeRecord, args=(f,))
    t1.start()
    t2.start()
    t1.join()
    # while x.is_alive():
