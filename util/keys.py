...
class Keys(object):
    ...
    mouse_absolute = 0x8000     # 判断是否绝对移动
    ...
    def directMouse(self, dx=0, dy=0, buttons=0, is_absolute=False):
        self.keys_worker.sendMouse(dx, dy, buttons, is_absolute)
...
class KeysWorker():
    ...
    # send mouse
    def sendMouse(self, dx, dy, buttons, is_absolute):
        if not is_absolute and dx != 0 or dy != 0:
            buttons |= self.keys.mouse_move
            # if is_absolute:
            #     buttons |= self.keys.mouse_absolute
...
if __name__ == '__main__':
    ...
    # keys.parseKeyString("a_down,-4,a_up")  # -4 - pause for 4 ms, 0x00 - hex code of Esc
    #
    # # queue of keys (virtual keys, threaded, only for keybord input)
    # keys.parseKeyString("vk,b_down,-4,b_up")  # -4 - pause for 4 ms
